...
namespace FTJ
{
	...
	CInputManager*  CInputManager::GetInstance() {
		if (!s_pInstance) {
			Console::Print("call to CInputManager::GetInstance() before being CreateInstance()");
			assert(0); // 
		}
		return s_pInstance;
	}
	...
	bool CInputManager::HandleWindowMessages(const UINT uMsg, const WPARAM wParam, const LPARAM lParam) {
		...
		case WM_SYSKEYDOWN: {
			...
			KeyDown(VK_LMENU);
			KeyDown(unsigned char(wParam & 0xFF));
			if (VK_RETURN == (unsigned char(wParam & 0xFF)) && !(lParam & 0xA0)) {
				//EventManager()->SendEvent("OnFullScreen",0,CEventManager::eEventInstant);
			}

			else if (VK_F4 == (unsigned char(wParam & 0xFF)) && !(lParam & 0xA0))
				PostQuitMessage(0);
			else
				break;
		}
	...
	void CInputManager::UpdateKeyStates(const float fDeltaTime) {
		...
		for (unsigned int i = 0; i < m_ActiveKeys.size(); ++i)
		{
			tKey& key = m_Keys[m_ActiveKeys[i]];
			switch (key.nKeyState)
			{
			case KEY_PRESS: {
				m_Keys[m_ActiveKeys[i]].nKeyState = KEY_DOWN;
				if (!m_ActiveKeyPressEvents[i].empty())
				{
					CEventManager::GetInstance()->SendEventNow(
						new CEvent(m_ActiveKeyPressEvents[i].c_str()));
				}
				break;
			}
			case KEY_DOWN: {
				...
				{
					CEventManager::GetInstance()->SendEventNow(
						new CEvent(m_ActiveKeyDownEvents[i].c_str(), &m_Keys[m_ActiveKeys[i]].fHeldDuration));
				}
				break;
			}
			case KEY_RELEASE: {
				...
				{
					CEventManager::GetInstance()->SendEventNow(
						new CEvent(m_ActiveKeyReleaseEvents[i].c_str()));
				}
				break;
			}
			}
