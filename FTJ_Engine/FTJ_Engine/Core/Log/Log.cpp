...
#include "DXLog.hpp"	//To Shut down DX debugging
...
namespace COMBINE(Log, __ID__) {
	...
	bool InitCommon() {
		...
		case 0:
			...
			break;
		case ENOENT:
			DebugPrint("Folder creation error, no file output");
			return false;
			break;
		case EEXIST:
			fileOutput = true;
			break;
		default:
			DebugPrint("unknown error code for _mkdir");
			return false;
		}
		AppData = szPath;
		AppData += "\\";
	...
	void LogLastError() {
		...
		Log("failed with error ", dw, " ", static_cast<LPTSTR>(lpMsgBuf) );
